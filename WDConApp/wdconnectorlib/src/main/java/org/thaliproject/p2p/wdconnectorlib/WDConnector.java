 ...
 */
public class WDConnector implements WifiBase.WifiStatusCallBack, WifiBase.HandShakerCallBack {

    WDConnector that = this;
    ...
    public WDConnector(Context Context, WDCallback Callback){
    ...
    private  void startHandShakerThread(String Address) {
        print_line("", "startHandShakerThread");
        stopHandShakerThread();
        mHandShakerThread = new HandShakerThread(that,Address,WifiBase.HandShakeportToUse);
        ...
        if(mWifiBase != null && list != null && list.size() > 0) {
            ...
            if(mWifiConnection != null){
                ...
                if (selItem != null) {
                    ...
                    print_line("", "Selected device. Address: " + selItem.deviceAddress + ", name: " + selItem.deviceName);
    ...
    public void GroupInfoAvailable(WifiP2pGroup group) {
        ...
        final WifiP2pGroup groupTmp = group;
        mHandler.post(new Runnable() {
            @Override
            public void run() {
                that.callback.GroupInfoChanged(groupTmp);
            }
        });
    }
    ...
    @Override
    public void connectionStatusChanged(WifiBase.ConectionState state, int Error) {
        ...
        if(state == WifiBase.ConectionState.NONE) {
            ...
            conStatus = "Connected";
            setConnectionState(ConnectionState.FetchingGOInfo);
        }else if(state == WifiBase.ConectionState.DisConnecting) {
            conStatus = "DisConnecting";
            setConnectionState(ConnectionState.Disconnecting);
        }else if(state == WifiBase.ConectionState.HandShaking) {
            conStatus = "HandShaking";
            setConnectionState(ConnectionState.HandShaking);
            ...
            // to make sure advertising is ok, lets celar the old out at this point
            stopWifiAccessPoint();
        ...
        print_line("COM", "State " + conStatus + " , Error: " + Error);
    }
    ...
    public void ConnectionFailed(String reason) {
        ...
        mHandler.post(new Runnable() {
            @Override
            public void run() {
                print_line("HandSS", "HandShake ConnectionFailed " + reasonTmp);
                connectionStatusChanged(WifiBase.ConectionState.ConnectingFailed,123456);
            }
        });
    }
